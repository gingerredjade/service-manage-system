      ...
      <b-alert show variant="primary" dismissible
        ...
        <div>
          ...
          <span class="hint-description"
            ...
            服务<span style="color: #007bff; font-weight: bold;">审核通过</span>，
            可<span style="color: #dc3545; font-weight: bold;">发布</span>
            &nbsp;<i class="fa fa-arrow-right" aria-hidden="true"></i>&nbsp;
            <span style="color: #dc3545; font-weight: bold;">浏览</span>
          </span>
        </div>
      </b-alert>
    </div>
            ...
            <div class="toolbar-content-condition">
              ...
              <div class="">
                ...
                <el-form-item prop="auditState">
                  <el-select v-model="filters.auditState" placeholder="选择审核状态">
                    <el-option
            ...
            <el-button-group>
              <el-tooltip :content="$t('ACTION.REFRESH')" placement="top">
                <el-button icon="fa fa-refresh"
                           @click="findPage({ pageRequest: {pageNum: 1, pageSize: pageRequest.pageSize} })"
                           :disabled="!hasPerms('svc:audit:view')"
                />
              </el-tooltip>
            </el-button-group>
          </el-form-item>
      ...
      <div class="kt-portlet__body kt-portlet__body&#45;&#45;fit">
        ...
        <div class="service-card-content">
          <el-table :data="pageResult.list" style="width: 100%" size="small" border
            ...
            <el-table-column label="审核状态" prop="auditState" header-align="center" align="center">
              <template slot-scope="scope">
            ...
            <el-table-column
              fixed="right" header-align="center" align="center" width="195" :label="$t('ACTION.OPERATION')">
              <template slot-scope="scope">
                <k-t-button icon="fa fa-edit" :label="$t('ACTION.AUDIT')" perms="svc:audit:audit" type="success" @click="handleAudit(scope.$index, scope.row)"/>
                <k-t-button icon="fa fa-trash" :label="$t('ACTION.DELETE')" perms="svc:audit:delete" type="danger" @click="handleDelete(scope.row)"/>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog :title="'审核'" width="40%"
      ...
      <el-form :model="dataForm" :rules="dataFormRules" ref="dataForm"
        ...
        <el-form-item label="审核状态" prop="auditState">
          <el-select v-model="dataForm.auditState" placeholder="请选择" style="width: 100%;">
            <el-option
              v-for="item in auditStateOptions"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="审核意见" prop="auditOpinion">
          <el-input v-model="dataForm.auditOpinion" auto-complete="off" type="textarea"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button :size="size" @click.native="handleCloseDialog()">{{$t('ACTION.CANCEL')}}</el-button>
        <el-button :size="size" type="primary" @click.native="submitAudit" :loading="editLoading">{{$t('ACTION.SUBMIT')}}</el-button>
      </div>
    </el-dialog>
          ...
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {
              ...
              if (this.dataForm.auditState === 0) {
                toAuditStatus = 'UNAUDITED';
              } else if (this.dataForm.auditState === 1) {
                toAuditStatus = 'AUDITED_PASS';
              } else if (this.dataForm.auditState === 2) {
                toAuditStatus = 'AUDITED_REFUSE';
              }
              let params = {id: this.dataForm.svcId, auditOpinion: this.dataForm.auditOpinion};
              this.$api.svc.updateAuditStatusAndOpinion(toAuditStatus, params).then( (res) => {
...
<style scoped>
  ...
  /*.service-card-content .card-image {
    width: 100%;
    display: block;
  }
  .clearfix:before,
